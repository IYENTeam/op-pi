...
2. **Project settings** (`.op-pi/settings.json` `permissions.*`).
3. **Global settings** (`~/.op-pi/agent/settings.json`).
4. **Session approvals** (in-memory + persisted to `~/.op-pi/agent/permissions/<project>.jsonl`).

...
## 2026-05-15 — stop rebuilding linked `op-pi` on launch

- Changed:
  - `scripts/build-all.mjs`
  - `scripts/create-root-op-pi-wrapper.mjs`
  - `scripts/create-root-op-pi-wrapper.test.mjs`
- Why: The PATH-visible `op-pi` command should not pay a build cost every time it starts. Build/link should create or refresh the shim, and regular launches should only execute the already-built CLI.
- What changed: Removed the git HEAD stamp, source mtime scan, dist marker check, and launch-time `scripts/build-all.mjs` call from the generated root wrapper. The build helper now also deletes the legacy `.op-pi-build-head` marker when refreshing `dist/op-pi`.
...
- Merge-conflict risk: low. The expected conflict zone is `scripts/create-root-op-pi-wrapper.mjs` if upstream changes local build/link behavior.

## 2026-05-15 — rebuild stale linked CLI before launching `op-pi`

- Changed:
  - `scripts/build-all.mjs`
  - `scripts/create-root-op-pi-wrapper.mjs`
  - `scripts/create-root-op-pi-wrapper.test.mjs`
- Why: The PATH-visible `op-pi` shim runs the root `dist/op-pi` wrapper. If source changes were committed but the workspace dist artifacts were not rebuilt, the linked command could still execute stale `packages/*/dist` code and reproduce fixed bugs.
- What changed: The root build writes the git HEAD it built into `dist/.op-pi-build-head`. The generated root wrapper now rebuilds when that stamp is missing or stale, when required dist markers are missing, or when relevant workspace source/package/script mtimes are newer than the build stamp. In a git checkout, if any check says the linked build is stale, it runs `scripts/build-all.mjs` before launching `packages/coding-agent/dist/op-pi`.
...
- Merge-conflict risk: low. The expected conflict zone is `scripts/create-root-op-pi-wrapper.mjs` if upstream changes the local build/link shim.
...
- Changed:
  ...
  - `README.md`
- Why: The todo tools now live in the public sibling `../pi-extensions/pi-todotools` repository, but op-pi should continue to ship them as a builtin.
...
## 2026-04-05 — add `op-pi` CLI alias

- Changed: `packages/coding-agent/package.json`
- Why: The user wants the built CLI to be directly runnable via `op-pi`. This cannot be implemented through the extension system because shell command exposure is controlled by the package `bin` map, not runtime extension hooks.
- What changed: Added a second CLI bin alias, `op-pi`, pointing at the existing `dist/cli.js` entrypoint alongside `pi`.
...
## 2026-04-12 — emit a callable `op-pi` artifact from the standard build

- Changed:
  - `packages/coding-agent/package.json`
  - `package.json`
  - `scripts/create-root-op-pi-wrapper.mjs`
- Why: The user wants root-level `npm run build` to be sufficient in the same practical sense that `op-pi` was: after building, there should be a directly callable `op-pi` command, not just an internal package artifact. A plain copied file in root `dist/` was not enough for `which op-pi`; the build also needed to refresh a PATH-visible shim.
- What changed:
  - Updated the coding-agent `build` script to emit `dist/op-pi` alongside `dist/cli.js`.
  - Updated the root `build` script to generate a root `dist/op-pi` wrapper that delegates to `packages/coding-agent/dist/cli.js`.
  - Added a small build helper at `scripts/create-root-op-pi-wrapper.mjs` to write that root wrapper.
  - Updated the root build helper to also write a small `op-pi` shim into npm's global `bin/` directory, so `which op-pi` resolves after a successful root build.
...
- Why: The original layout relied exclusively on npm's flat/hoisted install to satisfy cross-workspace transitive imports, and the root `build` / `check` scripts hardcoded `npm run X` while cd-ing through packages. That meant:
  - bun and pnpm both refused to install because several workspaces imported modules they did not declare as direct deps, and the root `package.json` still carried a stale `"@code-yeongyu/op-pi": "^0.30.2"` dependency from the rename from `@mariozechner/pi-coding-agent`.

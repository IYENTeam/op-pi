...
> ⚠️ **Experimental.** op-pi is an opinionated, in-flight fork of [badlogic/pi-mono](https://github.com/badlogic/pi-mono). It powers [Dori](https://sisyphuslabs.ai) under the hood and reflects what one specific AI assistant needs from a coding-agent runtime. Use it; don't bet a production pipeline on it.

op-pi is a senpai-name pun and a more **sane** pi with extra batteries included — a TypeScript monorepo that rebrands pi-mono's coding agent as `op-pi` and ships a curated set of builtin extensions and core tweaks on top of upstream.
...
op-pi was born from two influences:

- **Strong influence from [OMO (oh-my-openagent)](https://github.com/code-yeongyu/oh-my-openagent).** OMO is the heavyweight opencode harness with discipline agents (Sisyphus, Hephaestus, Prometheus), [IntentGate](https://factory.ai/news/terminal-bench), hash-anchored edits, Team Mode, skill-embedded MCPs, Ralph Loop, todo enforcers, and a lot more. op-pi reuses many of OMO's signature ideas (intent gate, dynamic prompt, per-model presets, parallel-tool routing, todo continuation) but **keeps the surface as light as possible** so it can stay close to upstream pi-mono. Think of op-pi as **a light version of OMO** that runs as a single pi CLI binary instead of an opencode plugin.
- **op-pi is the coding-agent runtime for [Dori](https://sisyphuslabs.ai).** Dori is Sisyphus Labs' AI assistant — see the [Dori callout in OMO's README](https://github.com/code-yeongyu/oh-my-openagent#oh-my-openagent) for context. op-pi is usable standalone, but the design decisions (intent gate phrasing, builtin extension set, prompt presets per model family, branding) are tuned for what Dori needs when it executes code work.
...
If you're migrating from [OMO (oh-my-openagent)](https://github.com/code-yeongyu/oh-my-openagent), here is the practical map from "what I used in OMO" to "what to install in op-pi". A lot of OMO's signature work is **already wired into op-pi as a builtin** (nothing to install); the rest lives in the [`code-yeongyu/pi-*`](https://github.com/code-yeongyu?tab=repositories&q=pi-) GitHub repos and is one `op-pi install` away.

> op-pi is intentionally lighter than OMO — single CLI binary, not an opencode plugin. Some OMO concepts (Discipline Agents, Team Mode, Skills, Hashline, Ralph Loop, `/init-deep`) have **no 1:1 op-pi counterpart** and are flagged at the bottom of this section. If you need those, keep using OMO.

### Already builtin in op-pi — nothing to install

| OMO feature | op-pi builtin |
|---|---|
...
### Install these pi-extension packages for the OMO-shaped op-pi

```bash
op-pi install git:github.com/code-yeongyu/pi-lsp-client            # 🛠️  LSP: rename / goto / refs / diagnostics + /lsp inspector
op-pi install git:github.com/code-yeongyu/pi-ast-grep              # 🛠️  AST-Grep across 25 languages (auto-downloads sg)
op-pi install git:github.com/code-yeongyu/pi-comment-checker       # 💬  Comment Checker — the standalone pi port of OMO's hook
op-pi install git:github.com/code-yeongyu/pi-rules                 #     Context injection: .claude/rules, .cursor/rules, .github/instructions, AGENTS.md, CLAUDE.md
op-pi install git:github.com/code-yeongyu/pi-nested-agents-md      # 🔍  Auto-injects nearby AGENTS.md (the runtime half of /init-deep)
op-pi install git:github.com/code-yeongyu/pi-websearch             # 📚  Provider-backed web search (fills the Exa-style slot)
op-pi install git:github.com/code-yeongyu/pi-webfetch              #     web_fetch tool (markdown / text / HTML, bounded time + size)
op-pi install git:github.com/code-yeongyu/pi-goal                  #     Persistent goal tracking + continuation prompts (closest thing to Sisyphus discipline)
op-pi install git:github.com/code-yeongyu/pi-sandbox               #     OS-level sandbox: native / Docker / justbash / QEMU + SSH transport

# Optional — only if you used the matching OMO surface:
op-pi install git:github.com/code-yeongyu/pi-cua-integration                 # 🖥️  Computer-use bindings (desktop / browser)
op-pi install git:github.com/code-yeongyu/pi-anthropic-computer-use          #     Claude-native computer use
op-pi install git:github.com/code-yeongyu/pi-anthropic-code-execution        #     Claude-native code execution sandbox
op-pi install git:github.com/code-yeongyu/pi-google-code-execution           #     Google-native code execution
op-pi install git:github.com/code-yeongyu/pi-openai-code-interpreter         #     OpenAI Code Interpreter
op-pi install git:github.com/code-yeongyu/pi-anthropic-text-editor           #     Claude-native text editor tool
op-pi install git:github.com/code-yeongyu/pi-anthropic-tool-search           #     Claude-native tool search
op-pi install git:github.com/code-yeongyu/pi-anthropic-web-fetch             #     Claude-native web fetch
op-pi install git:github.com/code-yeongyu/pi-google-google-search            #     Google Search grounding
op-pi install git:github.com/code-yeongyu/pi-google-url-context              #     Google URL grounding
op-pi install git:github.com/code-yeongyu/pi-openai-api-parallel-tool-calls  #     OpenAI parallel_tool_calls payload support
```

Each package is also installable by git URL, e.g. `op-pi install git:github.com/code-yeongyu/pi-comment-checker`. See [op-pi Packages](packages/coding-agent/README.md#pi-packages) for the full install / update / remove flow.

### No direct op-pi counterpart

These are part of OMO's opencode harness shape and are intentionally **not** in op-pi:
...
- **Hashline / hash-anchored edit tool** — op-pi stays with pi's standard `edit` / `multiedit` / `apply_patch`. `pi-comment-checker` covers the post-edit validation slot OMO uses Hashline for, just without `LINE#ID` content-hash identifiers.
- **Skill system** and **skill-embedded MCPs** — skills as a first-class concept (`SKILL.md`, scoped per-skill MCP servers) do not exist in op-pi.
...
op-pi ships a fixed set of builtin extensions and stops there. The [`code-yeongyu/pi-*`](https://github.com/code-yeongyu?tab=repositories&q=pi-) GitHub repos contain the full extension ecosystem: some packages are vendored into op-pi as builtins, while the rest can be installed on top when you want extra capabilities like LSP, AST-grep, sandboxing, goal tracking, web search/fetch, or rule loading.

### Installable extensions

These [`code-yeongyu/pi-*`](https://github.com/code-yeongyu?tab=repositories&q=pi-) packages are intended to be installed as standalone op-pi/pi extensions:
...
```bash
op-pi install git:github.com/code-yeongyu/pi-ast-grep
op-pi install git:github.com/code-yeongyu/pi-lsp-client
# ...substitute any of the package names from the table above.
```

See [op-pi Packages](packages/coding-agent/README.md#pi-packages) for the install / update / remove flow.

### pi-extension packages already shipped as op-pi builtins

You do **not** need to install these packages for normal op-pi use; their functionality is already included in the binary. This table is intentionally limited to packages that map to the currently registered `builtinExtensions` list:
...
Other builtins such as `permission-system`, `prompt-preset`, `anthropic-bash`, `service-tier`, `tool-pair-guard`, and `compaction` are op-pi-owned builtin extensions, not installable sibling packages.

## Why "op-pi"

`op-pi` is a small joke on **senpai**, but it is also literal project positioning: this fork aims to be a more **sane** pi with practical additions that make everyday agent work smoother without abandoning upstream's core design.
...
In-tree, tightly coupled to op-pi internals. Loaded in this exact registration order:
...
These are maintained as public `code-yeongyu/pi-*` repositories and can be installed through `settings.json` `packages` or `op-pi install git:github.com/code-yeongyu/<repo>`.
...
Not loaded as builtins; written once into `~/.op-pi/agent/extensions/` on first run so you can edit or remove them locally.
...
|---------|--------------|-----------|
| [`packages/ai`](packages/ai/AGENTS.md) | op-pi-branded Codex `originator` and User-Agent, shared tool-pair-repair utility, OpenAI Responses custom/freeform tool support for `apply_patch`, Claude Opus 4.7 + `"max"` thinking level, `extraBody` pass-through across every provider, [tool-call middleware rewrite](packages/ai/src/tool-call-middleware/AGENTS.md) | [`ai/src/changes.md`](packages/ai/src/changes.md) |
...
| [`packages/coding-agent`](packages/coding-agent/AGENTS.md) | `op-pi` branding, model config controls (`upstreamModelId`, `serviceTier`, favorite models, disable/whitelist/blacklist), non-blocking startup tool discovery, disabled startup update checks, [bash `promptSnippet` swap to `rg`](packages/coding-agent/src/core/tools/AGENTS.md), generated default extension fast-path, op-pi-branded outbound identity | Multiple [`changes.md`](packages/coding-agent/src/) files |

### Other branding / runtime changes

| Change | Details |
|--------|---------|
| **`op-pi` CLI identity** | The coding agent identifies itself as `op-pi`, uses `.op-pi/agent` for config storage, and publishes to npm as `@code-yeongyu/op-pi`. The installed executable remains `op-pi`. |
| **Senpi-branded outbound identity** | Every outbound request emits `op-pi` instead of `pi`: `User-Agent` (update check + Cloudflare + GitHub releases), `X-OpenRouter-Title`, OpenAI Codex `originator` + User-Agent. |
...
| **Self-update target** | `op-pi update op-pi` queries [`code-yeongyu/op-pi`](https://github.com/code-yeongyu/op-pi) releases, not upstream. |
...
3. **Remotes**: `origin` = [code-yeongyu/op-pi](https://github.com/code-yeongyu/op-pi), `upstream` = [badlogic/pi-mono](https://github.com/badlogic/pi-mono).
...
| `package.json` | Rebranded the coding agent package and runtime identity to `op-pi` |
...
| **[op-pi](packages/coding-agent)** | Interactive coding agent CLI, rebranded as op-pi |
...
npm run publish      # Publish npm workspaces, including @code-yeongyu/op-pi
```
...
## Ganbare Ganbare op-pi

> *Hora, op-pi op-pi, op-pi te kudasai!*
> *Ganbare ganbare op-pi, gan ganbare ganbare op-pi ora!*
...
- **C'mon op-pi, c'mon!** Ship the PR.
- **Notice me, op-pi.** ...the diagnostics noticed first.
- **Try harder, op-pi!** *(she did look. and the build did pass.)*
- **You can do it, op-pi!** One more agent, one more tool, one more clean rebase.
- **Ganbare, ganbare, op-pi! 頑張れ頑張れ先輩!** *Do your best, do your best, senpai!*

Yes, the entire project name is a senpai pun. Type strictly, run the tests, write a `changes.md`, keep the merge surface tiny — and *gan ganbare ganbare op-pi ora!*

Shoutout to [plugsuits](https://github.com/minpeter/plugsuits): op-pi's compaction system takes inspiration from its design.
